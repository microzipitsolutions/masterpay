...
test("one approved Pay-In among in-progress Pay-Ins earns positive commission and has 100% success", async () => {
  const db = await pool.connect();
  try {
    await db.query("BEGIN");
    const owner = (await db.query(`SELECT id, client_id FROM admins ORDER BY id LIMIT 1`)).rows[0];
    const suffix = crypto.randomBytes(5).toString("hex");
    const agent = (await db.query(
      `INSERT INTO agents(name,username,password,is_active,commission_percent,created_by_admin_id,client_id)
       VALUES($1,$1,'test',true,1,$2,$3) RETURNING id`,
      [`__commission_test_${suffix}`, owner.id, owner.client_id],
    )).rows[0];
    const ids = [];
    for (let i = 0; i < 132; i += 1) {
      ids.push((await db.query(
        `INSERT INTO transactions(transaction_id,amount,status,agent_id,created_by_admin_id,client_id)
         VALUES($1,100,'Pending',$2,$3,$4) RETURNING id`,
        [crypto.randomBytes(8).toString("hex"), agent.id, owner.id, owner.client_id],
      )).rows[0].id);
    }
    await db.query(`UPDATE transactions SET status='Approved' WHERE id=$1`, [ids[0]]);
    await db.query(`UPDATE transactions SET status='UTR Submitted' WHERE id=$1`, [ids[1]]);

    const row = (await db.query(
      `SELECT
         COUNT(*) FILTER (WHERE status IN ('Approved','Success'))::int AS successful,
         COUNT(*) FILTER (WHERE status IN ('Approved','Success','Rejected','Failed','Expired'))::int AS finalized,
         SUM(CASE WHEN agent_commission_amount >= 0 THEN agent_commission_amount
                  ELSE ROUND(amount * GREATEST(COALESCE(agent_commission_percent,0),0) / 100.0,2) END)
           FILTER (WHERE status IN ('Approved','Success')) AS commission
       FROM transactions WHERE agent_id=$1`, [agent.id],
    )).rows[0];
    assert.equal(row.successful, 1);
    assert.equal(row.finalized, 1);
    assert.equal((row.successful / row.finalized) * 100, 100);
    assert.equal(Number(row.commission), 1);
